## geotop_meteo_MakeMeteoSpinUpNARR_Daily.R
...
# if necessary, empirically correct incoming SW radiation
if (SW.corr){
  if (SW.corr.source=="TFS"){
    # read in data
    df.met <- read.csv(fname.TFS, stringsAsFactors=F)
    
    # format date
    df.met$Date <- dmy(df.met$Date)
    
    # convert to W m/2
    df.met$SWin.W.m2 <- as.numeric(df.met$Daily_globalrad_total_jcm2)*100*100/86400
    
  }
  if (SW.corr.source=="ARFlux"){
    # read in data
    df.met <- read.csv(fname.ARFlux, stringsAsFactors=F)
    
    # format date
    df.met$Date <- ymd(df.met$Date)
    
    # convert to W/m2
    #  -not necessary - already in W/m2

  }
  
  # make Year and DOY columns
  df.met$Year <- year(df.met$Date)
  df.met$DOY <- yday(df.met$Date)
  
  # trim to only complete cases
  df.met <- df.met[is.finite(df.met$SWin.W.m2), c("Year", "DOY", "SWin.W.m2")]
  
  # merge
  df.corr <- merge(df.met, df.in[,c("Year", "DOY", "SWin")], all.x=T)
  
  # rename columns
  colnames(df.corr) <- c("Year", "DOY", "met", "NARR")
  
  # # make plot
  # p.corr <-
  #   ggplot(df.corr, aes(y=met, x=NARR)) +
  #   geom_point() +
  #   stat_smooth(method="lm") +
  #   geom_abline(yintercept=0, slope=1, color="red")
  
  # calculate slope
  fit.corr <- lm(met ~ NARR, df.corr)
  
  # intercept & slope
  b.corr <- coef(fit.corr)[1]
  m.corr <- coef(fit.corr)[2]
  
  # apply correction
  df.in$SWin <- df.in$SWin*m.corr + b.corr
}
