## geotop_SoilPropertyCalibration_CreateGeotopInputFiles_WaterCalibration.R
#' This is intended to create a bunch of Geotop input files that will
#' be used for soil property calibration. Parameters varied for calibration 
#' are generated using Latin Hypercube Sampling (package lhs). For the _WaterCalibration
#' script, only soil hydraulic properties are varied.
...
out.path <- paste0(git.dir, "geotop/SoilPropertyCalibration/WaterCalibration/")
...
df.all <- data.frame(number = sprintf("%04d", seq(1,n.sample)),
                     ...
                     org.vwc_s = qunif(LHS.sample[,2], min=0.57, max=0.85),                                    # max observed VWC from ARFlux-Ameriflux data (3 sites)
                     org.vwc_r = qunif(LHS.sample[,3], min=0.03, max=0.10),                                    # min observed VWC from ARFlux-Ameriflux data
                     org.VG_alpha = qunif(LHS.sample[,4], min=2.50E-4, max=2.60E-03),                          # min/max of lit review for peat hydraulic properties, excluding max/min values
                     org.VG_n = qunif(LHS.sample[,5], min=1.11, max=4.8),                                      # min/max of lit review for peat hydraulic properties, excluding max/min values
                     org.thermcond = org.thermcond,
                     org.thermcap = org.thermcap,
