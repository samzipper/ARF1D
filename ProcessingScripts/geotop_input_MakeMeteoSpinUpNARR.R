...
if (SW.corr){
  # read in TFS data
  df.TFS <- read.csv(fname.TFS, stringsAsFactors=F)
  
  # make Year and DOY columns
  df.TFS$Date <- dmy(df.TFS$Date)
  df.TFS$Year <- year(df.TFS$Date)
  df.TFS$DOY <- yday(df.TFS$Date)
  
  # convert to W m/2
  df.TFS$SWin.W.m2 <- as.numeric(df.TFS$Daily_globalrad_total_jcm2)*100*100/86400
  
  # trim to only complete cases
  df.TFS <- df.TFS[is.finite(df.TFS$SWin.W.m2), c("Year", "DOY", "SWin.W.m2")]
  
  # merge
  df.corr <- merge(df.TFS, df.in[,c("Year", "DOY", "SWin")], all.x=T)
  
  # rename columns
  colnames(df.corr) <- c("Year", "DOY", "TFS", "NARR")
  
  # # make plot
  # p.corr <-
  #   ggplot(df.corr, aes(y=TFS, x=NARR)) +
  #   geom_point() +
  #   stat_smooth(method="lm") +
  #   geom_abline(yintercept=0, slope=1, color="red")
  
  # calculate slope
  fit.corr <- lm(TFS ~ NARR, df.corr)
