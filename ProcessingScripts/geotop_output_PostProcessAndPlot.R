...
df.mod.day <- subset(df.mod.day, year(Date)>=2008 & year(Date)<=2013)
df.mod.point <- subset(df.mod.point, year(Date)>=2008 & year(Date)<=2013)
df.thaw <- subset(df.thaw, year(Date)>=2008 & year(Date)<=2013)
...
p.thaw.compare.ARFlux <- 
  ggplot() +
  geom_line(data=df.mod.day, aes(x=Date, y=ThawDepth.mm), color="brown") +
...
p.precip.mm <- 
  ggplot(df.mod.point, aes(x=Date, xend=Date, y=0, yend=Prain_over_canopy.mm.+Psnow_over_canopy.mm.)) +
  geom_segment(color="blue") +
  scale_y_continuous(name="Precipitation (Rain + Snow) [mm]", expand=c(0,0), 
                     limits=c(0,max(df.mod.point$Prain_over_canopy.mm.+df.mod.point$Psnow_over_canopy.mm.)*1.05)) +
  scale_x_date(expand=c(0,0)) +
  theme_bw() +
  theme(panel.grid=element_blank())

p.snow.depth.mm <- 
  ggplot(df.mod.point, aes(x=Date, ymin=0, ymax=snow_depth.mm.)) +
  geom_ribbon(fill="deepskyblue1") +
  scale_y_continuous(name="Snow Depth [mm]", expand=c(0,0), 
                     limits=c(0,max(df.mod.point$snow_depth.mm.)*1.05)) +
  scale_x_date(expand=c(0,0)) +
  theme_bw() +
  theme(panel.grid=element_blank())

p.Tair.C <- 
  ggplot(df.mod.point, aes(x=Date, y=Tair.C.)) +
  geom_hline(yintercept=0, color="gray65") +
  geom_line(color="red") +
